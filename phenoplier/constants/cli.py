...
RUN_GLS_ARGS = ImmutableDict(
    {
        "input_file": "File path to S-MultiXcan result file (tab-separated and with at least columns 'gene' and "
                      "'pvalue').",
        "dup_genes_action": "Decide how to deal with duplicate gene entries in the input file. Mandatory if gene "
                            "identifies are duplicated.",
        "output_file": "File path where results will be written to.",
        "project_dir": "Path to the project directory that contains initialized settings files and data. If not "
                       "provided, will use the current shell directory.",
        "gene_corr_file": "Path to a gene correlations file or folder. It's is mandatory if running a GLS model, "
        ...
        "lv_list": "List of LV (gene modules) identifiers on which an association will be computed. All the rest not "
